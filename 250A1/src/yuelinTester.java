...
public class yuelinTester {
		...
		{	
			...
			try {
				...
				岳林队[3] = new Warrior(tm1m1, 60, "岳林方");
...
//				System.out.print(lou11.get(i).getHP()+"  ");
			...
			if(t02.isImproved()==true) System.out.println("Worker工作方式错误！！");
			岳林队[0].takeAction(t11);
			if(t11.isImproved()==false) System.out.println("Worker工作方式错误！！");
			
			岳林队[1].takeAction(t02);
			if(t02.isImproved()==true) System.out.println("Settelr工作方式错误！！");
			岳林队[1].takeAction(t11);
			if(t11.isImproved()==false) System.out.println("Settler工作方式错误！！");
			if(lou11.size()==8)  System.out.println("Settler工作方式错误！！");
			...
			if(岳林队[2].getHP()!=40) System.out.println("Archer工作方式(换箭)错误！！");
			宇婧队[2].takeAction(t11);
			for(int i=0;i<8;i++)
			System.out.print(岳林队[i].getHP()+"   ");
			if(岳林队[2].getHP()!=26.5) System.out.println("检查archer，注意防御10%加成，并且注意伤害没有提高5%");
			宇婧队[0].takeAction(t22);
		//	宇婧队[2].moveTo(t02);
			宇婧队[2].takeAction(t11); 
			if(岳林队[2].getHP()!=12.325) System.out.println("检查archer，并且注意伤害提高5%");
			/*System.out.println(lou11.size());
			for(int i = 0; i<lou11.size();i++)
				System.out.print(lou11.get(i).getHP()+"  ");*/
			宇婧队[2].takeAction(t11);
...
//			for(int i = 0; i<lou11.size();i++)
//				System.out.print(lou11.get(i).getHP()+"  ");
			if(t11.removeUnit(岳林队[2])==true) System.out.println("检查你的removeUnit");
